...
class RecordManager:
    ...
    def __init__(self, client: AtProtoClient):
        ...
        """
        self.client = client
        logger.info(f"Initialized RecordManager with client DID: {self.client.me.did if hasattr(self.client, 'me') else 'Not authenticated'}")
    ...
    def get_record(self, collection: str, rkey: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"Successfully retrieved record: {collection}/{rkey}")
            return response
        except Exception as e:
            if 'RecordNotFound' in str(e):
                logger.warning(f"Record not found: {collection}/{rkey}")
                return None
            else:
